...
Bind data and its copies retain an immutable declaration from the per-connection
`stream_bind_catalog`: catalog instance, stream ID, generation, full schema,
repository and expected senders. Physical planning consumes that declaration.

`declare(id, binding)` returns the published generation. Binding retains it via
`get_declaration(id)`; `get(id)` returns a detached snapshot. Physical lowering
records the source operator through `set_built()`, and the fragment retrieves it
with `get_built(id, generation)` to wire the session. A plan-owned attachment
clears only its matching declaration/operator pair when destroyed.

See [Shared Scan Framework](shared-scan-framework.md) for schema ownership and
source contracts.

### Contracts

- **Cleanup is generation-specific.** Fragments sharing a connection release their sessions
  and plans before calling `erase(id, generation)` for their own declarations, including after
  failed builds. `clear()` is reserved for callers that own the entire catalog.
- **Retained declarations cannot be replaced or removed.** Bind data and operator attachments
  block replacement, erase and clear with `stream_binding_in_use`. Clear checks every entry
  before removing any. Generation numbers never wrap or get reused.
...
**`build(query_id)`** first releases a failed attempt's sessions and plans, erases its
owned generations and redeclares the inputs. It then runs `plan_source` to obtain a
bound `LogicalOperator`, lowers it to a physical plan and roots it in a
...
- After planning, `get_built(id, generation)` must return an operator for every declared input.
  An unused input fails the build rather than leaving the session waiting on an unconnected stream.
